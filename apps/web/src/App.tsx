      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
        ...
        <aside className="w-72 bg-slate-900/40 backdrop-blur-3xl border-r border-slate-800 flex flex-col p-8 fixed h-full shadow-2xl">
          <div className="flex items-center gap-4 mb-12">
            ...
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">PlatformAccelerator</span>
          </div>
          
          <nav className="flex-1 space-y-2">
            <NavItem to="/" icon={<LayoutDashboard size={20} />} label="Executive Summary" active />
            <NavItem to="/portal" icon={<Book size={20} />} label="Developer Portal" />
            <NavItem to="/catalog" icon={<Search size={20} />} label="Service Catalog" />
            <NavItem to="/pipelines" icon={<Activity size={20} />} label="Delivery Hub" />
            <NavItem to="/governance" icon={<ShieldCheck size={20} />} label="Platform Governance" />
            <NavItem to="/productivity" icon={<Binary size={20} />} label="DX Intelligence" />
            <NavItem to="/finops" icon={<TrendingUp size={20} />} label="Platform FinOps" />
          </nav>
        ...
        <main className="flex-1 ml-72">
          <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-2 text-slate-400 text-sm font-medium uppercase tracking-widest">
              <span>Enterprise Platform Intelligence</span>
              <span>/</span>
              <span className="text-white font-bold">Internal Developer Platform</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-bold text-white">Platform Architect</p>
                <p className="text-[10px] text-indigo-400 uppercase tracking-widest font-black">Elite DX Engineering</p>
              </div>
